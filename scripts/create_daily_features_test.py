#!/usr/bin/env python3
"""
Daily Features Generator - Version 2 (TEST MODE)
...
TEST MODE: This script is configured to process ONLY player 614258
and outputs to a file with "_test" suffix for comparison purposes.
"""
...
def load_player_data(player_id: int, data_dir: str) -> Dict[str, pd.DataFrame]:
    ...
    TEST MODE: Loads ALL data first (profiles, injuries, career, matches),
    then filters to keep only data for the specified player_id.
    This ensures we don't miss any match data files.
    """
    logger.info(f"[TEST MODE] Loading ALL data first, then filtering to player {player_id}...")
...
def main():
    """Main entry point - TEST MODE: Processes only player 614258."""
    parser = argparse.ArgumentParser(description='Generate daily features for football players (TEST MODE: player 614258 only)')
    ...
    # Hardcode player ID for testing
    player_id = 614258
    
    logger.info("=" * 70)
    logger.info("DAILY FEATURES GENERATOR - TEST MODE")
    logger.info("=" * 70)
    logger.info(f"[TEST] Processing ONLY player {player_id}")
    ...
    # Output file path with _test suffix
    output_file = os.path.join(args.output_dir, f'player_{player_id}_daily_features_test.csv')
    
    # Check if file exists
    if os.path.exists(output_file) and not args.force:
        file_size = os.path.getsize(output_file) / (1024 * 1024)  # Size in MB
        logger.warning(f"File already exists: {output_file} ({file_size:.2f} MB)")
        logger.info("Use --force to regenerate")
        return
    elif os.path.exists(output_file) and args.force:
        logger.info(f"[FORCE] Regenerating existing file for player {player_id}")
    
    # Process single player
    logger.info(f"[TEST] Processing player {player_id}...")
    start_time = time.time()
    ...
    try:
        ...
        if daily_features.empty:
            logger.error(f"[ERROR] No features generated for player {player_id}")
            return
        ...
        logger.info("=" * 70)
        logger.info(f"[OK] Successfully generated features for player {player_id}")
        ...
        logger.info(f"   Rate: {len(daily_features)/elapsed:.0f} rows/sec")
        logger.info("=" * 70)
        
    except Exception as e:
        logger.error(f"[ERROR] Failed to generate features for player {player_id}")
