#!/usr/bin/env python3
"""
Train baseline models on V4 timeline datasets - Muscular Injuries Only (combined train+val)
- Training: 2022-07-01 to 2025-06-30 (combined train + validation datasets)
- Test: >= 2025-07-01
...
def apply_correlation_filter(X, threshold=0.8, cache_dir='cache', use_cache=True):
    ...
    Args:
        ...
        cache_dir: Directory for caching correlation matrix (default 'cache')
    ...
    if use_cache:
        ...
        # Create cache filename based on data shape and threshold
        cache_file = os.path.join(cache_dir, f'corr_matrix_{X.shape[0]}_{X.shape[1]}_{threshold}.npy')
        
        if os.path.exists(cache_file):
            print(f"   📦 Loading cached correlation matrix from {cache_file}...")
            try:
    ...
    if cache_file is None or not os.path.exists(cache_file):
        ...
        if use_cache and cache_file:
            print(f"   💾 Caching correlation matrix to {cache_file}...")
            try:
...
def train_gb(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    val_metrics = evaluate_model(gb_model, X_val, y_val, "Validation")
...
def train_xgb(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    val_metrics = evaluate_model(xgb_model, X_val, y_val, "Validation")
    test_metrics = evaluate_model(xgb_model, X_test, y_test, "Test")
    
    return xgb_model, train_metrics, val_metrics, test_metrics

def train_lgbm(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    val_metrics = evaluate_model(lgbm_model, X_val, y_val, "Validation")
    test_metrics = evaluate_model(lgbm_model, X_test, y_test, "Test")
    
    return lgbm_model, train_metrics, val_metrics, test_metrics

def main():
    # ========== CONFIGURATION ==========
    # Options for TARGET_RATIO: 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, or 'natural'
    TARGET_RATIO = 0.50  # Change this to switch between datasets
    CORR_THRESHOLD = 0.8
    ...
    if TARGET_RATIO == 'natural':
        if USE_POST2017_FILTER:
            ...
            ratio_display = 'Natural (unbalanced, post 2017-06-30)'
        ...
        if USE_POST2017_FILTER:
            ...
            ratio_display = f"{TARGET_RATIO:.0%} (balanced, post 2017-06-30)"
            ratio_title = f"{TARGET_RATIO:.0%} TARGET RATIO (POST 2017-06-30)"
        else:
            ratio_str = f"{int(TARGET_RATIO * 100):02d}pc"  # e.g., "05pc", "10pc", "25pc"
    ...
    print("   Training: 2021-07-01 to 2024-06-30 (seasons 2021/22, 2022/23, 2023/24)")
    ...
    print(f"📊 Estimated total time: 3-4.5 hours")
    ...
    test_file = 'timelines_35day_enhanced_natural_v4_muscular_test.csv'  # Always use natural test
    ...
    # Prepare data (with timing) - keep datasets separate
    print("\n📊 Preparing data...")
    ...
    # Apply correlation filter (with timing and caching)
    corr_start = datetime.now()
    selected_features = apply_correlation_filter(X_train, CORR_THRESHOLD, use_cache=True)
    ...
    for idx, (model_name, train_func, model_file_prefix) in enumerate(models_to_train, 1):
        ...
        # Train model
        model, train_metrics, val_metrics, test_metrics = train_func(
            X_train, y_train,
            X_val, y_val,
            X_test, y_test
        )
        ...
        # Save model
        model_file = f'models/{model_file_prefix}_model_v4_muscular_{ratio_str}_corr08.joblib'
        joblib.dump(model, model_file)
        
        # Save column names (REUSE the same serialized list - optimization!)
        columns_file = f'models/{model_file_prefix}_model_v4_muscular_{ratio_str}_corr08_columns.json'
    ...
    with open(f'experiments/v4_muscular_{ratio_str}_corr08_metrics.json', 'w', encoding='utf-8') as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)
    print(f"\n✅ Saved metrics to experiments/v4_muscular_{ratio_str}_corr08_metrics.json")
    ...
    for model_name in ['RF', 'GB', 'XGB', 'LGBM']:
        ...
        print(f"{'':<10} {'Validation':<30} {results['val']['precision']:<12.4f} {results['val']['recall']:<12.4f} {results['val']['f1']:<12.4f} {results['val']['roc_auc']:<12.4f} {results['val']['gini']:<12.4f}")
    ...
    summary_lines = [
        f"# V4 Timeline Datasets - Muscular Injuries Only - {ratio_title} Model Performance (with Correlation Filtering, threshold={CORR_THRESHOLD})",
        "",
        f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "",
        "## Dataset Split",
        "",
        f"- **Training:** 2021-07-01 to 2024-06-30 ({len(df_train):,} records, {df_train['target'].mean():.1%} injury ratio)",
    ...
    for model_name in ['RF', 'GB', 'XGB', 'LGBM']:
        ...
        summary_lines.append(f"| | Validation | {results['val']['precision']:.4f} | {results['val']['recall']:.4f} | {results['val']['f1']:.4f} | {results['val']['roc_auc']:.4f} | {results['val']['gini']:.4f} |")
    ...
    for model_name in ['RF', 'GB', 'XGB', 'LGBM']:
        ...
        summary_lines.append(f"- **F1 Gap (Train → Val):** {train_f1 - val_f1:.4f} ({(train_f1 - val_f1)/train_f1*100:.1f}% relative)" if train_f1 > 0 else "- **F1 Gap (Train → Val):** N/A")
        ...
        summary_lines.append(f"- **F1 Gap (Val → Test):** {val_f1 - test_f1:.4f} ({(val_f1 - test_f1)/val_f1*100:.1f}% relative)" if val_f1 > 0 else "- **F1 Gap (Val → Test):** N/A")
        summary_lines.append("")
    
    # Save summary
    summary_text = "\n".join(summary_lines)
    with open(f'experiments/v4_muscular_{ratio_str}_corr08_summary.md', 'w', encoding='utf-8') as f:
        f.write(summary_text)
    
    print(f"\n✅ Saved summary report to experiments/v4_muscular_{ratio_str}_corr08_summary.md")
