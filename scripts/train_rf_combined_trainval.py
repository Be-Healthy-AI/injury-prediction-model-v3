#!/usr/bin/env python3
"""
Train RF model on combined training + validation datasets
...
def main():
    print("="*80)
    print("TRAINING RF MODEL - COMBINED TRAINING + VALIDATION DATASETS")
    print("="*80)
    print("\n📋 Configuration:")
    ...
    if sample_weights is not None and len(sample_weights) == len(X_trainval):
        rf_model.fit(X_trainval, y_trainval, sample_weight=sample_weights)
    else:
    ...
    print("="*80)
    test_metrics = evaluate_model(rf_model, X_test, y_test, "Test (2025/26)", threshold=0.5)
    
    # Also evaluate on training set for comparison
    trainval_metrics = evaluate_model(rf_model, X_trainval, y_trainval, "Train+Val", threshold=0.5)
    
    # Save model
    os.makedirs('models', exist_ok=True)
    joblib.dump(rf_model, 'models/rf_model_combined_trainval.joblib')
    with open('models/rf_model_combined_trainval_columns.json', 'w') as f:
        json.dump(list(X_trainval.columns), f, indent=2)
    print(f"\n✅ Saved RF model to models/rf_model_combined_trainval.joblib")
    ...
    with open('experiments/rf_combined_trainval_metrics.json', 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"✅ Saved metrics to experiments/rf_combined_trainval_metrics.json")
    ...
    summary_text = "\n".join(summary_lines)
    with open('experiments/rf_combined_trainval_summary.md', 'w', encoding='utf-8') as f:
    ...
    print(f"✅ Summary saved to: experiments/rf_combined_trainval_summary.md")
