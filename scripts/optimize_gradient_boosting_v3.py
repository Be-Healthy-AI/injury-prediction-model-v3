#!/usr/bin/env python3
"""
Gradient Boosting Optimization Script V3
- Hyperparameter tuning for Gradient Boosting
- Feature selection to remove noisy/redundant features
- Goal: Improve F1-Score efficiently (focused search)
"""
import sys
import io
...
def get_feature_importance_baseline(X_train, y_train):
    """Get baseline feature importance from a quick Gradient Boosting"""
...
def tune_hyperparameters(X_train, y_train, cv_folds=3, n_iter=30):
    """Perform hyperparameter tuning using RandomizedSearchCV (focused search)"""
    print("\n🔧 Hyperparameter tuning...")
    print(f"   Using RandomizedSearchCV with {n_iter} iterations and {cv_folds}-fold CV")
    print(f"   Focused parameter space for efficiency...")
    
    # Define focused parameter grid (not too many combinations)
    param_distributions = {
    ...
    random_search = RandomizedSearchCV(
        ...
        n_iter=n_iter,  # Test 30 random combinations (focused)
        scoring=f1_scorer,
...
def main():
    ...
    # Test a couple of feature selection thresholds (quick)
    feature_selection_results = []
    
    for percentile in [10, 15]:  # Only test 2 thresholds for speed
        print(f"\n{'='*70}")
        ...
        test_model = GradientBoostingClassifier(
            n_estimators=100,  # Fewer for quick test
            max_depth=10,
            min_samples_split=10,
            min_samples_leaf=4,
            learning_rate=0.1,
    ...
    all_features_f1 = baseline_metrics['val_f1']
    best_feature_selection = max(feature_selection_results, key=lambda x: x['val_f1'])
    
    # Use best feature selection if it's better, otherwise use all features
    if best_feature_selection['val_f1'] > all_features_f1:
        ...
        selected_features = best_feature_selection['selected_features']
    else:
        print(f"\n✅ Using all features (best performance)")
    ...
    best_model, best_params, best_cv_f1 = tune_hyperparameters(X_train_optimized, y_train, cv_folds=3, n_iter=30)
    ...
    if optimized_metrics['val_f1'] > baseline_metrics['val_f1']:
        ...
        X_optimized_full = X_encoded[selected_features]
        ...
        model_file = 'models/model_v3_gradient_boosting_optimized.pkl'
        columns_file = 'models/model_v3_gb_optimized_training_columns.json'
        params_file = 'models/model_v3_gb_optimized_params.json'
        
        joblib.dump(final_model, model_file)
        json.dump(selected_features, open(columns_file, 'w'))
