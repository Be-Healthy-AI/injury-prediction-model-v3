#!/usr/bin/env python3
"""
Threshold Optimization for Combined Train+Val GB Model
...
def main():
    print("="*80)
    print("THRESHOLD OPTIMIZATION - COMBINED TRAIN+VAL GB MODEL")
    print("="*80)
    ...
    y_proba_trainval = gb_model.predict_proba(X_trainval)[:, 1]
    ...
    for threshold in thresholds:
        # Train+Val
        trainval_metrics = evaluate_threshold(y_trainval, y_proba_trainval, threshold)
        trainval_metrics['threshold'] = threshold
        results['trainval'].append(trainval_metrics)
    ...
    print("\n" + "="*80)
    print("BEST OPERATING POINTS (TEST DATASET)")
    print("="*80)
    ...
    with open('experiments/combined_gb_threshold_optimization.json', 'w') as f:
        json.dump(output_data, f, indent=2)
    print(f"\n✅ Saved detailed results to: experiments/combined_gb_threshold_optimization.json")
    ...
    summary_text = "\n".join(summary_lines)
    with open('experiments/combined_gb_threshold_optimization_summary.md', 'w', encoding='utf-8') as f:
        f.write(summary_text)
    
    print(f"✅ Summary saved to: experiments/combined_gb_threshold_optimization_summary.md")
