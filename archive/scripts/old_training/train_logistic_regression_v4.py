#!/usr/bin/env python3
"""
Logistic Regression Training Script V4 with Temporal Split
...
def prepare_data(df):
    """Prepare data with same encoding logic as V3, excluding week_5 features"""
    # Exclude metadata columns and all week_5 features (using 28-day window instead of 35-day)
...
def main():
    print("🚀 LOGISTIC REGRESSION TRAINING V4 (DUAL VALIDATION)")
    print("=" * 70)
    ...
    # Prepare data
    print("\n🔧 Preparing data...")
    X_train_full, y_train_full, _ = prepare_data(df_train_full)
    X_val_outsample, y_val_outsample, _ = prepare_data(df_val_outsample)
    ...
    print("=" * 70)
    val_insample_metrics, _, _ = evaluate_model(lr_model, X_val_insample, y_val_insample, "In-Sample Validation")
    ...
    print("=" * 70)
    val_outsample_metrics, _, _ = evaluate_model(lr_model, X_val_outsample, y_val_outsample, "Out-of-Sample Validation")
    ...
    model_file = 'models/lr_model_v4.joblib'
    columns_file = 'models/lr_model_v4_columns.json'
    metrics_file = 'models/lr_model_v4_metrics.json'
