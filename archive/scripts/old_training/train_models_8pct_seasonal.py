#!/usr/bin/env python3
"""
Train models on 8% target ratio seasonal split datasets
- Training: <= 2024-06-30 (from train file)
- Validation: 2024/25 season (from val file)
- Test: 2025/26 season (from test file)
"""
import sys
import io
...
def main():
    print("="*80)
    print("TRAINING MODELS - 8% TARGET RATIO SEASONAL SPLIT")
    print("="*80)
    print("\n📋 Dataset Configuration:")
    print("   Training: <= 2024-06-30 (from train file)")
    print("   Validation: 2024/25 season (from val file)")
    print("   Test: 2025/26 season (from test file)")
    print("   Target Ratio: 8% for all datasets")
    print("="*80)
    ...
    if df_train['target'].nunique() < 2:
        print(f"\n⚠️  WARNING: Training data has only {df_train['target'].nunique()} class(es)")
        print(f"   This will cause issues with model training.")
        print(f"   Training injury ratio: {df_train['target'].mean():.1%}")
        if df_train['target'].mean() == 1.0:
            print(f"   ⚠️  Training data is 100% injuries - models cannot learn from this!")
            print(f"   Consider regenerating timelines with different parameters.")
            return
    ...
    joblib.dump(rf_model, 'models/rf_model_8pct_seasonal.joblib')
    with open('models/rf_model_8pct_seasonal_columns.json', 'w') as f:
        json.dump(list(X_train.columns), f, indent=2)
    print(f"\n✅ Saved RF model to models/rf_model_8pct_seasonal.joblib")
    ...
    # Save GB model
    joblib.dump(gb_model, 'models/gb_model_8pct_seasonal.joblib')
    with open('models/gb_model_8pct_seasonal_columns.json', 'w') as f:
        json.dump(list(X_train.columns), f, indent=2)
    print(f"\n✅ Saved GB model to models/gb_model_8pct_seasonal.joblib")
    ...
    # Save LR model
    joblib.dump(lr_model, 'models/lr_model_8pct_seasonal.joblib')
    with open('models/lr_model_8pct_seasonal_columns.json', 'w') as f:
        json.dump(list(X_train.columns), f, indent=2)
    print(f"\n✅ Saved LR model to models/lr_model_8pct_seasonal.joblib")
    
    # Save metrics
    with open('experiments/8pct_seasonal_metrics.json', 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"\n✅ Saved metrics to experiments/8pct_seasonal_metrics.json")
    ...
    summary_lines = [
        ...
        f"- **Validation:** 2024/25 season ({len(df_val):,} records, {df_val['target'].mean():.1%} injury ratio)",
    ...
    # Save summary
    summary_text = "\n".join(summary_lines)
    with open('experiments/8pct_seasonal_summary.md', 'w', encoding='utf-8') as f:
    ...
    for model_name in ['RF', 'GB', 'LR']:
        ...
        for dataset_name, metrics in [('Training', results['train']), 
                                       ('Validation (2024/25)', results['validation']), 
                                       ('Test (2025/26)', results['test'])]:
            cm = metrics['confusion_matrix']
            table_lines.append(
                f"| {dataset_name} | {metrics['accuracy']:.4f} | {metrics['precision']:.4f} | "
                f"{metrics['recall']:.4f} | {metrics['f1']:.4f} | {metrics['roc_auc']:.4f} | "
                f"{metrics['gini']:.4f} | {cm['tp']} | {cm['fp']} | {cm['tn']} | {cm['fn']} |"
            )
        table_lines.append("")
        if model_name != 'LR':
            table_lines.append(f"## {model_name} (GB)" if model_name == 'GB' else f"## {model_name}")
            table_lines.append("")
            table_lines.append("| Dataset | Accuracy | Precision | Recall | F1-Score | ROC AUC | Gini | TP | FP | TN | FN |")
            table_lines.append("|---------|----------|-----------|--------|----------|---------|------|----|----|----|----|")
    
    table_text = "\n".join(table_lines)
    with open('experiments/8pct_seasonal_performance_table.md', 'w', encoding='utf-8') as f:
        f.write(table_text)
    
    total_time = datetime.now() - start_time
    print(f"\n✅ Complete! Total time: {total_time}")
    print(f"✅ Summary saved to: experiments/8pct_seasonal_summary.md")
    print(f"✅ Detailed table saved to: experiments/8pct_seasonal_performance_table.md")
