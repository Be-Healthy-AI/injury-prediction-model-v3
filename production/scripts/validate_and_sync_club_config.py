#!/usr/bin/env python3
"""
Validate and sync club config.json files with raw data and Transfermarkt.

This script:
1. Validates that players in config.json have required raw data
2. Identifies players with raw data but missing from config
3. Optionally syncs with Transfermarkt to detect transfers
4. Can auto-fix config.json by adding/removing players

Usage:
    # Report mode (default) - identifies issues but doesn't change configs
    python validate_and_sync_club_config.py --country England --club "AFC Bournemouth" --data-date 20260114
    
    # Auto-fix mode - automatically updates config.json
    python validate_and_sync_club_config.py --country England --club "AFC Bournemouth" --data-date 20260114 --auto-fix
    
    # With Transfermarkt sync
    python validate_and_sync_club_config.py --country England --club "AFC Bournemouth" --data-date 20260114 --sync-transfermarkt --auto-fix
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate and sync club config.json files'
    )
    parser.add_argument(
        '--country',
        type=str,
