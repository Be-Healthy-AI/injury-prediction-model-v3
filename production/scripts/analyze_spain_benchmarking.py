#!/usr/bin/env python3
"""
CD Leganés Benchmarking Analysis
Compares CD Leganés with other LaLiga2 clubs across multiple KPIs.
...
Output: CSV files and summary report in spain/20260103_CD Leganés Benchmarking/
"""
...
def get_latest_data_folder(country: str) -> Optional[Path]:
    ...
    country_dir = PRODUCTION_ROOT / "raw_data" / country.lower()
...
DATA_DIR = get_latest_data_folder("spain")
if DATA_DIR is None:
    raise ValueError("No data folder found for Spain. Please run fetch_spain_injuries.py first.")

OUTPUT_DIR = DATA_DIR.parent / f"{DATA_DIR.name}_CD Leganés Benchmarking"
...
SEASONS = {
    ...
    "2025/26": {
        ...
        "year": 2025,
        "league": "LaLiga2",  # Second division
        "competition_slug": "segunda-division",
        "competition_id": "ES2"
    }
}
...
# LaLiga clubs for 2024/25 season (first division - 20 clubs)
LALIGA_CLUBS_2024_25 = {
    "Athletic Bilbao",
    "Atlético de Madrid",
    "CA Osasuna",
    "CD Leganés",
...
# LaLiga2 clubs for 2025/26 season (second division - 22 clubs)
LALIGA2_CLUBS_2025_26 = {
...
def fetch_players_from_transfermarkt(
    ...
    for club_idx, club in enumerate(clubs, 1):
        ...
        if club_name == 'Real Sociedad B' and season_name == '2025/26':
            should_exclude = False
...
def identify_players_from_match_data(
    ...
    # previous_seasons is at spain level (same level as 20260103 folder)
    ...
    for player_id in all_player_ids:
        ...
        if not profile_row.empty:
            ...
            if current_club and current_club.strip():
                ...
                if is_target_club(current_club, target_clubs):
...
def is_target_club(club_name: str, target_clubs: Set[str]) -> bool:
    ...
    for target_club in target_clubs:
        ...
        # Handle "CD" vs "Club Deportivo" variations
        if club_name_lower.startswith('cd ') and target_club_lower.startswith('club deportivo '):
            if club_name_lower[3:] == target_club_lower[15:]:
                return True
        if club_name_lower.startswith('club deportivo ') and target_club_lower.startswith('cd '):
            if club_name_lower[15:] == target_club_lower[3:]:
                return True
...
def is_target_league_club(club_name: str, season_name: str) -> bool:
    """Check if a club is in the target league for the season (LaLiga for 2024/25, LaLiga2 for 2025/26)."""
    if pd.isna(club_name) or club_name == '':
        return False
    
    # Get target clubs for the season
    if season_name == "2024/25":
        target_clubs = LALIGA_CLUBS_2024_25
    elif season_name == "2025/26":
        target_clubs = LALIGA2_CLUBS_2025_26
    else:
        return False
    
    return is_target_club(club_name, target_clubs)
...
def map_players_to_clubs_by_season(
    ...
    if scraper and season_info:
        try:
            ...
            # Filter to only target players if specified
...
def calculate_injury_weeks(
    ...
    analysis_end: pd.Timestamp,    # Today or season end
    injuries_df: pd.DataFrame
) -> Tuple[int, int, int]:
    """
    Calculate injured and non-injured weeks for a player since analysis_start.
    Only includes injuries that occurred or were ongoing since analysis_start.
    If a player was already injured at analysis_start, count that injury.
    ...
    if player_injuries.empty:
        ...
        total_weeks = calculate_weeks_between(analysis_start, analysis_end)
    ...
    # Filter injuries that overlap with the analysis period
    injury_periods = []
    for _, injury in player_injuries.iterrows():
        ...
        # If end date is missing, use analysis end
        if pd.isna(injury_end):
            injury_end = analysis_end
        
        # Include injury if:
        # 1. It started on or after analysis_start, OR
        # 2. It was ongoing at analysis_start (started before but ended after or is still ongoing)
        if injury_start >= analysis_start:
            # Injury started during analysis period
            injury_start_clipped = injury_start
            injury_end_clipped = min(injury_end, analysis_end)
        elif injury_end >= analysis_start:
            # Injury was ongoing at analysis_start
            injury_start_clipped = analysis_start
            injury_end_clipped = min(injury_end, analysis_end)
        else:
            # Injury ended before analysis_start - skip it
            continue
    ...
    if not injury_periods:
        # No injuries in analysis period
        total_weeks = calculate_weeks_between(analysis_start, analysis_end)
        return (0, total_weeks, total_weeks)
    
    # Divide analysis period into weeks
    injured_weeks = set()
    current_date = analysis_start
    
    while current_date <= analysis_end:
        week_start = current_date
        week_end = min(current_date + timedelta(days=6), analysis_end)
        
        if is_week_injured(week_start, week_end, injury_periods):
            # Calculate week number (0-indexed)
            week_num = (current_date - analysis_start).days // 7
...
def aggregate_club_kpis(player_level_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (club, season), group in player_level_df.groupby(['club', 'season']):
        ...
        # For LaLiga2, "Real Sociedad B" might actually be a first team club
        if club_str == 'Real Sociedad B' and season == '2025/26':
            should_exclude = False
...
def generate_benchmarking_report(club_level_df: pd.DataFrame, output_dir: Path):
    ...
    report_lines = []
    report_lines.append("# CD Leganés Benchmarking Analysis Report")
    report_lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report_lines.append("")
    report_lines.append("=" * 80)
    report_lines.append("")
    
    # Filter CD Leganés data
    leganes_data = club_level_df[club_level_df['club'].str.contains('Leganés|Leganes', case=False, na=False)]
    other_clubs = club_level_df[~club_level_df['club'].str.contains('Leganés|Leganes', case=False, na=False)]
    
    for season in ['2024/25', '2025/26']:
        report_lines.append(f"## Season {season}")
        report_lines.append("")
        
        season_leganes = leganes_data[leganes_data['season'] == season]
        season_others = other_clubs[other_clubs['season'] == season]
        
        if season_leganes.empty:
            report_lines.append(f"  No data for CD Leganés in {season}")
            report_lines.append("")
            continue
        
        leganes_row = season_leganes.iloc[0]
        ...
        league_name = "LaLiga" if season == "2024/25" else "LaLiga2"
        report_lines.append(f"### CD Leganés vs League Averages ({league_name} only)")
        report_lines.append("")
        report_lines.append(f"**Injury Rate:**")
        report_lines.append(f"  - CD Leganés: {leganes_row['club_injury_rate_pct']:.2f}%")
        report_lines.append(f"  - League Average: {league_avg_injury_rate:.2f}%")
        report_lines.append(f"  - Difference: {leganes_row['club_injury_rate_pct'] - league_avg_injury_rate:+.2f}%")
        report_lines.append("")
        
        report_lines.append(f"**Average Age:**")
        if pd.notna(leganes_row['average_age']):
            report_lines.append(f"  - CD Leganés: {leganes_row['average_age']:.1f} years")
        if pd.notna(league_avg_age):
            report_lines.append(f"  - League Average: {league_avg_age:.1f} years")
            if pd.notna(leganes_row['average_age']):
                report_lines.append(f"  - Difference: {leganes_row['average_age'] - league_avg_age:+.1f} years")
        report_lines.append("")
        
        report_lines.append(f"**Total Market Value:**")
        report_lines.append(f"  - CD Leganés: €{leganes_row['total_market_value']:,.0f}")
        report_lines.append(f"  - League Average: €{league_avg_market_value:,.0f}")
        report_lines.append(f"  - Difference: €{leganes_row['total_market_value'] - league_avg_market_value:+,.0f}")
        report_lines.append("")
        ...
        leganes_rank_injury = (sorted_injury['club_injury_rate_pct'] < leganes_row['club_injury_rate_pct']).sum() + 1
        total_clubs = len(season_others_target) + 1
        report_lines.append(f"**Injury Rate:** CD Leganés ranks #{leganes_rank_injury} out of {total_clubs} clubs (lower is better)")
        report_lines.append("")
        
        # Market value ranking (higher is better)
        sorted_market = season_others_target.sort_values('total_market_value', ascending=False)
        leganes_rank_market = (sorted_market['total_market_value'] > leganes_row['total_market_value']).sum() + 1
        report_lines.append(f"**Market Value:** CD Leganés ranks #{leganes_rank_market} out of {total_clubs} clubs (higher is better)")
        report_lines.append("")
...
def main():
    print("=" * 80)
    print("CD Leganés Benchmarking Analysis")
    print("=" * 80)
    print()
    ...
    try:
        ...
        # Season 2024/25: LaLiga (first division)
        season_2024_25 = SEASONS["2024/25"]
        print(f"\n{'='*80}")
        print(f"Processing Season: 2024/25 (LaLiga - First Division)")
        print(f"{'='*80}")
        
        print(f"Identifying players in LaLiga clubs for 2024/25...")
        laliga_players = identify_players_in_league_clubs(
            profiles_df,
            career_df,
        ...
        print(f"  Found {len(laliga_players)} players in LaLiga clubs")
        ...
        print(f"Calculating KPIs for {len(player_clubs_2024_25)} LaLiga players...")
        ...
        for player_id, club_info in player_clubs_2024_25.items():
            ...
            # Calculate injury weeks (from 2025-07-01 onwards)
        ...
        # Season 2025/26: LaLiga2 (second division)
        season_2025_26 = SEASONS["2025/26"]
        print(f"\n{'='*80}")
        print(f"Processing Season: 2025/26 (LaLiga2 - Second Division)")
        print(f"{'='*80}")
        
        print(f"Identifying players in LaLiga2 clubs for 2025/26...")
        laliga2_players = identify_players_in_league_clubs(
            profiles_df,
            career_df,
        ...
        print(f"  Found {len(laliga2_players)} players in LaLiga2 clubs")
        ...
        print(f"Calculating KPIs for {len(player_clubs_2025_26)} LaLiga2 players...")
        ...
        # CD Leganés focus comparison
        leganes_focus = club_level_df.copy()
        leganes_clubs = leganes_focus[leganes_focus['club'].str.contains('Leganés|Leganes', case=False, na=False)]['club'].unique()
        
        if len(leganes_clubs) > 0:
            leganes_focus_path = OUTPUT_DIR / "benchmarking_cd_leganes_focus.csv"
            leganes_focus.to_csv(leganes_focus_path, index=False, encoding='utf-8-sig', sep=';')
            print(f"  CD Leganés focus: {leganes_focus_path}")
