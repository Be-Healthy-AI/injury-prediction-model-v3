#!/usr/bin/env python3
"""
Generate daily injury predictions for production using lgbm_muscular_v3 model.

Reads timelines from production/deployments/{country}/{club}/timelines/ and generates
predictions using the lgbm_muscular_v3 winner model.
...
# Note: V3 uses the same preprocessing as V2 (verified compatible)
...
# Model paths
MODEL_DIR = PRODUCTION_ROOT / "models" / "lgbm_muscular_v3"
MODEL_PATH = MODEL_DIR / "model.joblib"
COLUMNS_PATH = MODEL_DIR / "columns.json"


def load_model():
    """Load the lgbm_muscular_v3 model and feature columns."""
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate daily injury predictions using lgbm_muscular_v3 model'
    )
    parser.add_argument(
        '--country',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output file path (default: predictions_lgbm_v3_YYYYMMDD.csv in predictions folder)'
    )
    parser.add_argument(
        '--cache-dir',
        type=str,
    ...
    # Determine output file - prioritize data_date parameter if provided
    if args.output_file:
        output_file = Path(args.output_file)
    else:
        # If data_date is provided, use it for filename (ensures correct date in filename)
        if args.data_date:
            date_str = args.data_date
            print(f"[INFO] Using data_date parameter for filename: {date_str}")
        else:
            # Fallback: Get max reference_date from timelines
        ...
        output_file = predictions_dir / f"predictions_lgbm_v3_{date_str}.csv"
    ...
    # Use separate cache for V3 to avoid conflicts with V2
    # Include club name in cache filename to avoid conflicts between clubs
    club_safe = args.club.replace(" ", "_").replace("/", "_")
    cache_file = cache_dir / f"preprocessed_v3_{club_safe}_{timelines_file.stem}.csv"
    
    print("=" * 80)
    print("GENERATE PREDICTIONS - lgbm_muscular_v3")
    print("=" * 80)
    ...
    # Get date_str for per-player filenames - use data_date if provided, otherwise max reference_date
    if not args.data_date:
        # Only recalculate if data_date was not provided
        ...
        # Use the same date_str that was set for the aggregated file (from args.data_date)
        print(f"[INFO] Using data_date parameter for per-player files: {date_str}")
    ...
    for player_id in predictions['player_id'].unique():
        ...
        # Save per-player CSV (with v3 suffix to distinguish from v2)
        player_csv = players_dir / f"player_{player_id}_predictions_v3_{date_str}.csv"
