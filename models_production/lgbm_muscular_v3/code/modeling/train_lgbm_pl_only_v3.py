#!/usr/bin/env python3
"""
Train LGBM V3 model on PL-only filtered timelines.

- Target: muscular injuries only, 35-day horizon
- Data: 10% target ratio, seasonal combined datasets, ALL seasons (PL-only)
...
def main() -> None:
    ...
    ratio_title = (
        f"{TARGET_RATIO:.0%} TARGET RATIO - LGBM V3 PL-ONLY (ALL SEASONS, INCLUDING 2025-2026)"
    )
    ...
    # ------------------------------------------------------------------
    print("\n📂 Loading PL-only timeline data (ALL seasons, 10% ratio)...")
    ...
    if PREPROCESS_CACHE:
        ...
        cache_file = f"{CACHE_DIR}/preprocessed_v3_pl_only_{hash_key}.csv"
    else:
        cache_file = None
    ...
    # ------------------------------------------------------------------
    # 4) TRAIN FINAL MODEL ON ALL DATA AND SAVE AS v3
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("TRAIN FINAL LGBM MODEL ON ALL PL-ONLY DATA (v3)")
    print("=" * 80)
    ...
    print("\n🚀 Training final LGBM v3 model on ALL PL-only data...")
    ...
    train_metrics = evaluate_model(final_model, X_all, y_all, "All Seasons PL-Only (Train)")

    # Save training metrics for auditability
    v3_model_dir = v3_root / "model"
    v3_model_dir.mkdir(parents=True, exist_ok=True)
    
    train_metrics_path = v3_model_dir / "lgbm_v3_pl_only_metrics_train.json"
    ...
    # Save model + columns as v3 artifacts
    ...
    print(f"\n✅ Saved final LGBM v3 model to {model_path}")
    ...
    print(
        ...
        "   - Compare metrics with v1/v2 to assess impact of PL-only filtering"
    )
