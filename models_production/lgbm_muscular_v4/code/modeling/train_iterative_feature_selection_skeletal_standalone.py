#!/usr/bin/env python3
"""
Iterative Feature Selection Training for Model 2 (Skeletal Injuries Only) - Standalone
...
2. Trains Model 2 (Skeletal) iteratively with increasing feature sets (20, 40, 60, ...)
3. Tracks performance metrics (Gini and F1-Score on test set) for Model 2 only
4. Stops when 3 consecutive drops in performance are detected
5. Identifies the optimal number of features for Model 2

Performance Metric: weighted combination of Gini coefficient and F1-Score for Model 2
...
try:
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write(f"=== Iterative Training Log (Model 2 - Skeletal) Started at {datetime.now().isoformat()} ===\n")
...
def filter_timelines_for_model(timelines_df: pd.DataFrame, target_column: str) -> pd.DataFrame:
    """
    Filter timelines for Model 2 (skeletal injuries), excluding muscular injuries from negatives.
    
    This ensures Model 2 only learns to distinguish skeletal injuries from non-injuries,
    not from muscular injuries.
    
    Only includes timelines where target2=1 (positives)
    or (target1=0 AND target2=0) (negatives - only non-injuries)
    
    Args:
        timelines_df: DataFrame with target1 and target2 columns
        target_column: Must be 'target2' for this Model 2-only script
        
    Returns:
        Filtered DataFrame with only relevant timelines for Model 2
    """
    if target_column != 'target2':
        raise ValueError(f"This script is for Model 2 only. target_column must be 'target2', got: {target_column}")
...
def train_model_with_feature_subset(feature_subset, verbose=True):
    ...
    if verbose:
        log_message(f"\n{'='*80}")
        log_message(f"TRAINING MODEL 2 (SKELETAL) WITH {len(feature_subset)} FEATURES")
        log_message(f"{'='*80}")
    ...
    features_used = sorted(list(X_train_skeletal.columns))
    ...
    try:
        ...
        lgbm_model.fit(X_train_skeletal, y_train_skeletal)
        train_metrics = evaluate_model(lgbm_model, X_train_skeletal, y_train_skeletal, "Training")
        test_metrics = evaluate_model(lgbm_model, X_test_skeletal, y_test_skeletal, "Test")
    except Exception as e:
...
def calculate_combined_score(test_metrics, gini_weight=GINI_WEIGHT, f1_weight=F1_WEIGHT):
    """
    Calculate weighted combination of Gini and F1-Score for Model 2 (Skeletal).
    
    Args:
        test_metrics: Test metrics for Model 2 (skeletal)
        gini_weight: Weight for Gini coefficient (default 0.6)
        f1_weight: Weight for F1-Score (default 0.4)
    
    Returns:
        Combined performance score for Model 2
    """
    try:
        combined_score = (
...
def run_iterative_training():
    ...
    log_message(f"   Performance metric: {GINI_WEIGHT} * Gini + {F1_WEIGHT} * F1-Score (Model 2 only)")
    ...
    while True:
        ...
        log_message(f"\n{'='*80}")
        log_message(f"ITERATION {iteration}: Training Model 2 with {n_features} features")
        log_message(f"{'='*80}")
        ...
        try:
            ...
            # Calculate combined performance score (Model 2 only)
            ...
            log_message(f"   Model 2 (Skeletal) - Test: Gini={training_results['test_metrics']['gini']:.4f}, "
    ...
    try:
        ...
        if len(combined_scores) > 1:
            ...
            plt.title('Model 2 (Skeletal) Performance vs Number of Features', fontsize=14, fontweight='bold')
            plt.grid(True, alpha=0.3)
            plt.legend()
            plt.tight_layout()
            
            plot_file = MODEL_OUTPUT_DIR / 'iterative_feature_selection_skeletal_plot.png'
