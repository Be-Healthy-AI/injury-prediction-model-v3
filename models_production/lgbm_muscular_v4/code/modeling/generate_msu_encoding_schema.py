#!/usr/bin/env python3
"""
Generate encoding_schema.json for model_msu_lgbm from columns.json and a timeline CSV.

Use this when you have an existing model_msu_lgbm deployment but no encoding_schema.json.
Run from repo root.

Usage:
  python models_production/lgbm_muscular_v4/code/modeling/generate_msu_encoding_schema.py
  python models_production/lgbm_muscular_v4/code/modeling/generate_msu_encoding_schema.py --timelines path/to/timelines.csv
"""
...
MODEL_MSU_DIR = ROOT_DIR / "models_production" / "lgbm_muscular_v4" / "model_msu_lgbm"
EXCLUDED_RAW_FEATURES = ('current_club', 'current_club_country', 'previous_club', 'previous_club_country')


def main():
    parser = argparse.ArgumentParser(description="Generate encoding_schema.json for model_msu_lgbm")
    parser.add_argument(
        "--timelines",
        type=Path,
        default=None,
        help="Path to a timeline CSV (same columns as training). Default: test folder MSU d7 or muscular.",
    )
    args = parser.parse_args()

    cols_path = MODEL_MSU_DIR / "columns.json"
    ...
    if args.timelines is not None:
        ...
        msu_d7 = TEST_DIR / "timelines_35day_season_2025_2026_v4_labeled_msu_d7.csv"
        ...
        if msu_d7.exists():
            ...
            print(
                f"ERROR: No default timelines found. Tried {msu_d7}, {d7}, {v4_test}. Use --timelines PATH.",
                file=sys.stderr,
            )
            return 1
    ...
    meta = ['player_id', 'reference_date', 'date', 'player_name', 'target1', 'target2', 'target', 'target_msu', 'has_minimum_activity']
    feature_cols = [c for c in df.columns if c not in meta and c not in EXCLUDED_RAW_FEATURES]
    ...
    out_path = MODEL_MSU_DIR / "encoding_schema.json"
    MODEL_MSU_DIR.mkdir(parents=True, exist_ok=True)
