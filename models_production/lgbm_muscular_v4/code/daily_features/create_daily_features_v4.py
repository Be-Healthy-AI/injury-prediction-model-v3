...
def transform_injury_recency_features(injury_df: pd.DataFrame) -> pd.DataFrame:
    ...
    by applying log(1 + x) transformation to handle -1 values (no injury) and 0 values.
    ...
    for feature in injury_recency_features:
        if feature in transformed_df.columns:
            # Log transform: log(1 + max(x, 0)) to handle -1 (no injury) and 0 values
            # This ensures:
            #   -1 (no injury) -> log(1 + 0) = 0
            #   0 (injury today) -> log(1 + 0) = 0
            #   1 day -> log(2) ≈ 0.69
            #   30 days -> log(31) ≈ 3.43
            #   365 days -> log(366) ≈ 5.90
            #   730 days -> log(731) ≈ 6.59
            original_values = transformed_df[feature].copy()
            transformed_df[feature] = np.log1p(np.maximum(transformed_df[feature], 0))
            transformed_count += 1
