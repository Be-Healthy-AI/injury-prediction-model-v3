#!/usr/bin/env python3
"""
Daily Features Generator - Version 4 Enhanced
Simplified feature set (60-80 features) with enhanced workload, recovery, and composite indicators.
Key improvements:
- Simplified injury recency features (5-7 core features)
...
- Removed complex/redundant features
"""
...
def determine_calendar(
    ...
    career: Optional[pd.DataFrame] = None
) -> pd.DatetimeIndex:
    """
...
def calculate_match_features(
    ...
    feature_names = [
        ...
        'teams_this_season', 'teams_last_season', 'teams_season_today',
        # V4 Enhanced: Removed 'season_team_diversity'
        ...
        # V4 Enhanced: Removed 'national_team_intensity', 'competition_intensity', 'competition_pressure',
        # 'competition_experience', 'competition_diversity', 'competition_frequency'
        # V4 Enhanced: Keep 'competition_level', 'international_competitions', 'cup_competitions'
        'competition_level', 'international_competitions', 'cup_competitions',
        ...
        'substitution_minutes_played',
        # V4 Enhanced: Removed 'substitution_efficiency', 'substitution_mood_indicator'
        ...
        'cum_team_points',
        # V4 Enhanced: Removed 'team_points_rolling5', 'team_mood_score'
        'cum_matches_injured'
    ]
    ...
    for i, date in enumerate(calendar):
        ...
        # V4 Enhanced: Removed season_team_diversity calculation
        ...
        # Competition analysis (simplified - keep only competition_level, international_competitions, cup_competitions)
        if not past_matches.empty:
            comp_importances = past_matches['competition_importance'].dropna()
            if len(comp_importances) > 0:
                # V4 Enhanced: Keep competition_level, removed competition_intensity
                features['competition_level'][i] = comp_importances.max()
            
            # V4 Enhanced: Removed competition_diversity, competition_frequency, competition_experience, competition_pressure
            
            # International and cup competitions (keep these)
        ...
        # V4 Enhanced: Removed substitution_efficiency calculation
...
def calculate_injury_features(injuries: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        features = {
            ...
            'days_since_last_muscular': [-1] * n_days,
            'days_since_last_skeletal': [-1] * n_days,
            # V4 Enhanced: Removed granular recency features (days_since_last_unknown, days_since_last_other, etc.)
            ...
            'illness_count': [0] * n_days,
            'unknown_body_part_count': [0] * n_days,
            # V4 Enhanced: Removed granular body part recency features
            ...
            'severe_injury_count': [0] * n_days,
            'critical_injury_count': [0] * n_days,
            # V4 Enhanced: Removed granular severity recency features
        ...
        for i, date in enumerate(calendar):
            ...
            if len(past_injuries) > 0:
                ...
                # V4 Enhanced: Only calculate recency for core classes (muscular, skeletal)
                # Removed granular recency calculations for unknown, other, body parts, severity
                for class_name in ['muscular', 'skeletal']:
...
def transform_injury_recency_features(injury_df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply log transformation and capping to CORE injury recency features only.
    
    V4 Enhanced: Only transform 4 core features, cap at 730 days (2 years).
    ...
    """
    logger.info("=== Applying log transformation to CORE injury recency features ===")
    
    # V4 Enhanced: Only transform 4 core features (simplified)
    core_injury_recency_features = [
    ...
    # Apply log transformation and capping to each core feature
    transformed_count = 0
    for feature in core_injury_recency_features:
        if feature in transformed_df.columns:
            ...
            # V4 Enhanced: Cap at 730 days (2 years) before log transformation
            # Preserve -1 values, cap non-negative values at 730, then transform
            mask = transformed_df[feature] >= 0
            transformed_df.loc[mask, feature] = transformed_df.loc[mask, feature].clip(upper=730)
    ...
    logger.info(f"Applied log transformation to {transformed_count} core injury recency features")
    return transformed_df

def add_injury_category_features(injury_df: pd.DataFrame, injuries: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Add simplified injury category features:
    - injury_free_period_category (recent/medium/long)
    - injuries_last_2_years, muscular_injuries_last_2_years, skeletal_injuries_last_2_years
    - last_injury_severity
    """
    logger.info("=== Adding injury category features ===")
    n_days = len(calendar)
    
    # Initialize new features
    injury_df['injury_free_period_category'] = 'long'  # Default
    injury_df['injuries_last_2_years'] = 0
    injury_df['muscular_injuries_last_2_years'] = 0
    injury_df['skeletal_injuries_last_2_years'] = 0
    injury_df['last_injury_severity'] = 0
    
    if not injuries.empty and 'fromDate' in injuries.columns:
        # Pre-process injuries
        injuries['start_norm'] = pd.to_datetime(injuries['fromDate']).dt.normalize()
        injuries['severity_numeric'] = injuries['severity'].apply(map_severity_to_numeric)
        
        for i, date in enumerate(calendar):
            date_norm = date.normalize()
            days_since_last = injury_df.loc[date, 'days_since_last_injury'] if 'days_since_last_injury' in injury_df.columns else -1
            
            # Categorize injury-free period
            if days_since_last == -1:
                injury_df.loc[date, 'injury_free_period_category'] = 'long'
            elif days_since_last < 180:
                injury_df.loc[date, 'injury_free_period_category'] = 'recent'
            elif days_since_last < 730:
                injury_df.loc[date, 'injury_free_period_category'] = 'medium'
            else:
                injury_df.loc[date, 'injury_free_period_category'] = 'long'
            
            # Count injuries in last 2 years (730 days)
            cutoff_date = date_norm - timedelta(days=730)
            recent_injuries = injuries[
                (injuries['start_norm'] > cutoff_date) & 
                (injuries['start_norm'] <= date_norm)
            ]
            
            injury_df.loc[date, 'injuries_last_2_years'] = len(recent_injuries)
            
            # Count by class
            muscular_recent = recent_injuries[
                recent_injuries['injury_class'].astype(str).str.lower().fillna('') == 'muscular'
            ]
            skeletal_recent = recent_injuries[
                recent_injuries['injury_class'].astype(str).str.lower().fillna('') == 'skeletal'
            ]
            
            injury_df.loc[date, 'muscular_injuries_last_2_years'] = len(muscular_recent)
            injury_df.loc[date, 'skeletal_injuries_last_2_years'] = len(skeletal_recent)
            
            # Last injury severity (if within 2 years)
            if days_since_last >= 0 and days_since_last < 730 and len(recent_injuries) > 0:
                last_injury = recent_injuries.iloc[-1]
                injury_df.loc[date, 'last_injury_severity'] = last_injury.get('severity_numeric', 0)
    
    logger.info("=== Completed adding injury category features ===")
    return injury_df

def calculate_workload_acceleration(match_df: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Calculate workload acceleration features.
    Internal calculations: minutes_last_7d, minutes_previous_7d, etc.
    Stored features: acceleration and spike indicators only.
    """
    logger.info("=== Calculating workload acceleration features ===")
    n_days = len(calendar)
    
    features = {
        'workload_acceleration_7d': [0.0] * n_days,
        'workload_acceleration_14d': [0.0] * n_days,
        'workload_spike_7d': [0] * n_days,
        'workload_spike_14d': [0] * n_days,
    }
    
    if not match_df.empty and 'minutes_played_numeric' in match_df.columns:
        # match_df should have DatetimeIndex from calculate_match_features
        for i, date in enumerate(calendar):
            date_norm = date.normalize()
            
            # Calculate rolling sums internally (not stored)
            date_end = date_norm
            date_start_7d = date_norm - timedelta(days=6)  # Last 7 days
            date_start_14d = date_norm - timedelta(days=13)  # Last 14 days
            date_start_prev_7d = date_start_7d - timedelta(days=7)  # Previous 7 days
            date_start_prev_14d = date_start_14d - timedelta(days=14)  # Previous 14 days
            
            # Get minutes in each period (match_df has DatetimeIndex)
            if isinstance(match_df.index, pd.DatetimeIndex):
                minutes_last_7d = match_df[
                    (match_df.index >= date_start_7d) & 
                    (match_df.index <= date_end)
                ]['minutes_played_numeric'].sum()
                
                minutes_previous_7d = match_df[
                    (match_df.index >= date_start_prev_7d) & 
                    (match_df.index < date_start_7d)
                ]['minutes_played_numeric'].sum()
                
                minutes_last_14d = match_df[
                    (match_df.index >= date_start_14d) & 
                    (match_df.index <= date_end)
                ]['minutes_played_numeric'].sum()
                
                minutes_previous_14d = match_df[
                    (match_df.index >= date_start_prev_14d) & 
                    (match_df.index < date_start_14d)
                ]['minutes_played_numeric'].sum()
            else:
                # Fallback: if index is not DatetimeIndex, use date column
                minutes_last_7d = 0
                minutes_previous_7d = 0
                minutes_last_14d = 0
                minutes_previous_14d = 0
            
            # Calculate acceleration
            if minutes_previous_7d > 0:
                features['workload_acceleration_7d'][i] = (minutes_last_7d - minutes_previous_7d) / minutes_previous_7d
                features['workload_spike_7d'][i] = 1 if features['workload_acceleration_7d'][i] > 0.3 else 0
            
            if minutes_previous_14d > 0:
                features['workload_acceleration_14d'][i] = (minutes_last_14d - minutes_previous_14d) / minutes_previous_14d
                features['workload_spike_14d'][i] = 1 if features['workload_acceleration_14d'][i] > 0.3 else 0
    
    logger.info("=== Completed calculating workload acceleration features ===")
    return pd.DataFrame(features, index=calendar)

def calculate_recent_match_frequency(matches: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Calculate recent match frequency features (counts, not sums).
    """
    logger.info("=== Calculating recent match frequency features ===")
    n_days = len(calendar)
    
    features = {
        'matches_in_last_7_days': [0] * n_days,
        'matches_in_last_14_days': [0] * n_days,
        'matches_in_last_21_days': [0] * n_days,
        'matches_in_last_30_days': [0] * n_days,
    }
    
    if not matches.empty and 'date' in matches.columns:
        matches['date_norm'] = pd.to_datetime(matches['date']).dt.normalize()
        
        for i, date in enumerate(calendar):
            date_norm = date.normalize()
            
            # Count matches in each window
            for window_days, feature_name in [(7, 'matches_in_last_7_days'), 
                                               (14, 'matches_in_last_14_days'),
                                               (21, 'matches_in_last_21_days'),
                                               (30, 'matches_in_last_30_days')]:
                window_start = date_norm - timedelta(days=window_days-1)
                matches_in_window = matches[
                    (matches['date_norm'] >= window_start) & 
                    (matches['date_norm'] <= date_norm) &
                    (matches['matches_played'] > 0)
                ]
                features[feature_name][i] = len(matches_in_window)
    
    logger.info("=== Completed calculating recent match frequency features ===")
    return pd.DataFrame(features, index=calendar)

def calculate_recovery_time(matches: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Calculate recovery time between matches.
    """
    logger.info("=== Calculating recovery time features ===")
    n_days = len(calendar)
    
    features = {
        'recovery_time_avg_last_3': [0.0] * n_days,
        'recovery_time_min_last_3': [0] * n_days,
        'short_recovery_matches_14d': [0] * n_days,
    }
    
    if not matches.empty and 'date' in matches.columns:
        matches['date_norm'] = pd.to_datetime(matches['date']).dt.normalize()
        matches_sorted = matches.sort_values('date_norm')
        
        for i, date in enumerate(calendar):
            date_norm = date.normalize()
            
            # Get matches up to this date where player actually played
            past_matches = matches_sorted[
                (matches_sorted['date_norm'] <= date_norm) &
                (matches_sorted['matches_played'] > 0)
            ]
            
            if len(past_matches) >= 2:
                # Get last 3 matches
                last_3_matches = past_matches.tail(3)
                match_dates = pd.to_datetime(last_3_matches['date_norm']).sort_values()
                
                # Calculate recovery times
                recovery_times = []
                for j in range(1, len(match_dates)):
                    recovery_days = (match_dates.iloc[j] - match_dates.iloc[j-1]).days
                    recovery_times.append(recovery_days)
                
                if recovery_times:
                    features['recovery_time_avg_last_3'][i] = np.mean(recovery_times)
                    features['recovery_time_min_last_3'][i] = int(min(recovery_times))
            
            # Count short recovery matches in last 14 days
            matches_14d = past_matches[past_matches['date_norm'] > (date_norm - timedelta(days=14))]
            if len(matches_14d) >= 2:
                match_dates_14d = pd.to_datetime(matches_14d['date_norm']).sort_values()
                short_recovery_count = 0
                for j in range(1, len(match_dates_14d)):
                    recovery_days = (match_dates_14d.iloc[j] - match_dates_14d.iloc[j-1]).days
                    if recovery_days < 3:
                        short_recovery_count += 1
                features['short_recovery_matches_14d'][i] = short_recovery_count
    
    logger.info("=== Completed calculating recovery time features ===")
    return pd.DataFrame(features, index=calendar)

def calculate_recent_patterns(matches: pd.DataFrame, calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Calculate recent pattern features (substitutions, bench time, high intensity).
    """
    logger.info("=== Calculating recent pattern features ===")
    n_days = len(calendar)
    
    features = {
        'early_substitutions_last_7d': [0] * n_days,
        'early_substitutions_last_14d': [0] * n_days,
        'consecutive_early_substitutions': [0] * n_days,
        'matches_bench_last_7d': [0] * n_days,
        'matches_bench_last_14d': [0] * n_days,
        'bench_ratio_last_14d': [0.0] * n_days,
        'high_intensity_matches_14d': [0] * n_days,
        'consecutive_high_intensity': [0] * n_days,
    }
    
    if not matches.empty:
        matches['date_norm'] = pd.to_datetime(matches['date']).dt.normalize() if 'date' in matches.columns else matches.index
        if 'competition_importance' not in matches.columns:
            matches['competition_importance'] = 0
        
        for i, date in enumerate(calendar):
            date_norm = date.normalize()
            
            # Get matches in recent windows
            matches_7d = matches[
                (matches['date_norm'] > (date_norm - timedelta(days=7))) &
                (matches['date_norm'] <= date_norm)
            ]
            matches_14d = matches[
                (matches['date_norm'] > (date_norm - timedelta(days=14))) &
                (matches['date_norm'] <= date_norm)
            ]
            
            # Early substitutions (substituted off before minute 60)
            if 'substitutions_off' in matches.columns:
                early_subs_7d = matches_7d[
                    (matches_7d['substitutions_off'].notna()) &
                    (matches_7d['substitutions_off'] < 60)
                ]
                early_subs_14d = matches_14d[
                    (matches_14d['substitutions_off'].notna()) &
                    (matches_14d['substitutions_off'] < 60)
                ]
                features['early_substitutions_last_7d'][i] = len(early_subs_7d)
                features['early_substitutions_last_14d'][i] = len(early_subs_14d)
                
                # Consecutive early substitutions - RELAXED CONDITION
                # Changed from >= 2 to >= 1 to capture more cases
                if len(early_subs_14d) >= 1:  # Changed from >= 2
                    early_subs_sorted = early_subs_14d.sort_values('date_norm')
                    consecutive = 0
                    for j in range(1, len(early_subs_sorted)):
                        days_diff = (early_subs_sorted.iloc[j]['date_norm'] - early_subs_sorted.iloc[j-1]['date_norm']).days
                        if days_diff <= 7:  # Within a week
                            consecutive += 1
                    features['consecutive_early_substitutions'][i] = consecutive
                else:
                    features['consecutive_early_substitutions'][i] = 0
            
            # Bench time
            if 'matches_bench_unused' in matches.columns:
                bench_7d = matches_7d[matches_7d['matches_bench_unused'] > 0]
                bench_14d = matches_14d[matches_14d['matches_bench_unused'] > 0]
                features['matches_bench_last_7d'][i] = len(bench_7d)
                features['matches_bench_last_14d'][i] = len(bench_14d)
                
                # Bench ratio
                total_matches_14d = len(matches_14d[matches_14d['matches_played'] > 0])
                if total_matches_14d > 0:
                    features['bench_ratio_last_14d'][i] = len(bench_14d) / total_matches_14d
            
            # High intensity matches (competition_importance > 0.7, assuming 0-1 scale)
            # Adjust threshold if competition_importance uses different scale
            high_intensity_14d = matches_14d[
                (matches_14d['competition_importance'] > 0.7) |
                (matches_14d['competition_importance'] > 3)  # Handle 0-5 scale
            ]
            features['high_intensity_matches_14d'][i] = len(high_intensity_14d)
            
            # Consecutive high intensity
            if len(high_intensity_14d) >= 2:
                high_intensity_sorted = high_intensity_14d.sort_values('date_norm')
                consecutive = 0
                for j in range(1, len(high_intensity_sorted)):
                    days_diff = (high_intensity_sorted.iloc[j]['date_norm'] - high_intensity_sorted.iloc[j-1]['date_norm']).days
                    if days_diff <= 7:  # Within a week
                        consecutive += 1
                features['consecutive_high_intensity'][i] = consecutive
    
    logger.info("=== Completed calculating recent pattern features ===")
    return pd.DataFrame(features, index=calendar)
...
def calculate_composite_indicators(
    ...
    """
    logger.info("=== Calculating composite indicators ===")
    n_days = len(calendar)
    
    features = {
        'fatigue_indicator': [0.0] * n_days,
        'workload_x_injury_history': [0.0] * n_days,
        'age_workload_risk': [0.0] * n_days,
    }
    
    for i, date in enumerate(calendar):
        # Fatigue indicator (composite of multiple signals)
        fatigue_score = 0.0
        
        # High minutes in last 7 days (need to calculate from match_df)
        if 'minutes_played_numeric' in match_df.columns:
            date_norm = date.normalize()
            date_start_7d = date_norm - timedelta(days=6)
            if isinstance(match_df.index, pd.DatetimeIndex):
                minutes_last_7d = match_df[
                    (match_df.index >= date_start_7d) & 
                    (match_df.index <= date_norm)
                ]['minutes_played_numeric'].sum()
            else:
                minutes_last_7d = 0  # Will calculate from matches if needed
            fatigue_score += (minutes_last_7d > 450) * 0.25
        
        # Workload spike
        if 'workload_spike_7d' in workload_df.columns:
            fatigue_score += workload_df.loc[date, 'workload_spike_7d'] * 0.25
        
        # Short recovery
        if 'short_recovery_matches_14d' in recovery_df.columns:
            fatigue_score += (recovery_df.loc[date, 'short_recovery_matches_14d'] >= 2) * 0.25
        
        # Early substitutions
        if 'early_substitutions_last_7d' in pattern_df.columns:
            fatigue_score += (pattern_df.loc[date, 'early_substitutions_last_7d'] >= 1) * 0.25
        
        features['fatigue_indicator'][i] = fatigue_score
        
        # Workload × Injury History interaction
        if 'injuries_last_2_years' in injury_df.columns:
            date_norm = date.normalize()
            date_start_14d = date_norm - timedelta(days=13)
            if isinstance(match_df.index, pd.DatetimeIndex):
                minutes_last_14d = match_df[
                    (match_df.index >= date_start_14d) & 
                    (match_df.index <= date_norm)
            ...
            injuries_last_2_years = injury_df.loc[date, 'injuries_last_2_years']
            days_since_muscular = injury_df.loc[date, 'days_since_last_muscular'] if 'days_since_last_muscular' in injury_df.columns else 730
            
            injury_factor = 1.0 if injuries_last_2_years > 0 else 0.5
            muscular_factor = 1.0 if days_since_muscular < 180 else 0.7
            
            features['workload_x_injury_history'][i] = (minutes_last_14d * injury_factor * muscular_factor) / 1000.0
        
        # Age × Workload interaction
        if 'age' in profile_df.columns:
            age = profile_df.loc[date, 'age']
            date_norm = date.normalize()
            date_start_14d = date_norm - timedelta(days=13)
            if isinstance(match_df.index, pd.DatetimeIndex):
                minutes_last_14d = match_df[
            ...
            features['age_workload_risk'][i] = (age / 30.0) * (minutes_last_14d / 630.0)
    
    logger.info("=== Completed calculating composite indicators ===")
    return pd.DataFrame(features, index=calendar)

def calculate_temporal_features(calendar: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Calculate temporal/seasonal features.
    """
    logger.info("=== Calculating temporal features ===")
    n_days = len(calendar)
    
    features = {
        # REMOVED: 'season_year' - temporal leak, should not be used in model
        'season_month': [0] * n_days,
        'is_pre_season': [0] * n_days,
        'is_mid_season': [0] * n_days,
        'is_end_season': [0] * n_days,
        'days_into_season': [0] * n_days,
        'is_early_season': [0] * n_days,  # NEW: First 30 days of season
    }
    
    for i, date in enumerate(calendar):
        # REMOVED: features['season_year'][i] = date.year  # Temporal leak
        features['season_month'][i] = date.month
        
        # Season phases
        month = date.month
        features['is_pre_season'][i] = 1 if month in [7, 8] else 0
        features['is_mid_season'][i] = 1 if month in [9, 10, 11, 12, 1, 2, 3, 4] else 0
        features['is_end_season'][i] = 1 if month in [5, 6] else 0
        
        # Days into season (assuming season starts July 1)
        if month >= 7:
        ...
        features['days_into_season'][i] = days_into_season
        
        # NEW: Early season indicator (first 30 days)
        features['is_early_season'][i] = 1 if days_into_season <= 30 else 0
    
    logger.info("=== Completed calculating temporal features ===")
    return pd.DataFrame(features, index=calendar)
...
def generate_daily_features_for_player(
    ...
    calendar = determine_calendar(matches, injuries, reference_date, player_row, career)
    step_time = time.time() - step_start
    ...
    injury_df = transform_injury_recency_features(injury_df)  # V4 Enhanced: Apply log transformation to core features only
    step_time = time.time() - step_start
    logger.debug(f"[Step 6.5] Transformation completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 6.6] Adding injury category features...")
    injury_df = add_injury_category_features(injury_df, injuries, calendar)  # V4 Enhanced: Add simplified category features
    step_time = time.time() - step_start
    logger.debug(f"[Step 6.6] Injury category features completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7] Calculating workload acceleration features...")
    workload_df = calculate_workload_acceleration(match_df, calendar)  # V4 Enhanced: New workload features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7] Workload acceleration features: {len(workload_df)} rows × {len(workload_df.columns)} columns | Completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7.1] Calculating recent match frequency features...")
    match_freq_df = calculate_recent_match_frequency(matches, calendar)  # V4 Enhanced: New match frequency features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7.1] Recent match frequency features completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7.2] Calculating recovery time features...")
    recovery_df = calculate_recovery_time(matches, calendar)  # V4 Enhanced: New recovery time features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7.2] Recovery time features completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7.3] Calculating recent pattern features...")
    pattern_df = calculate_recent_patterns(matches, calendar)  # V4 Enhanced: New pattern features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7.3] Recent pattern features completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7.4] Calculating composite indicators...")
    composite_df = calculate_composite_indicators(workload_df, recovery_df, pattern_df, injury_df, match_df, profile_df, calendar)  # V4 Enhanced: New composite features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7.4] Composite indicators completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 7.5] Calculating temporal features...")
    temporal_df = calculate_temporal_features(calendar)  # V4 Enhanced: New temporal features
    step_time = time.time() - step_start
    logger.debug(f"[Step 7.5] Temporal features completed in {step_time:.2f} seconds")
    
    step_start = time.time()
    logger.info("[Step 8] Calculating interaction features...")
    interaction_df = calculate_interaction_features(profile_df, match_df, injury_df)
    step_time = time.time() - step_start
    logger.debug(f"[Step 8] Interaction features: {len(interaction_df)} rows × {len(interaction_df.columns)} columns | Completed in {step_time:.2f} seconds")
    
    # Combine all features
    step_start = time.time()
    logger.info("[Step 9] Combining all features...")
    daily_features = pd.concat([
        profile_df, match_df, injury_df, interaction_df,
        workload_df, match_freq_df, recovery_df, pattern_df, composite_df, temporal_df
    ], axis=1)  # V4 Enhanced: Include all new feature DataFrames
    step_time = time.time() - step_start
