...
if sys.platform == 'win32':
    ...
    # Only modify stdout/stderr if they are valid and not already TextIOWrappers
    # Skip modification if we're in a context where it might fail (e.g., subprocess, closed streams)
    if sys.stdout is not None and sys.stdout is not sys.__stdout__:
        # Already modified, skip
        pass
    elif hasattr(sys.stdout, 'buffer'):
        try:
            # Test if we can actually access and use the buffer
            test_buffer = sys.stdout.buffer
            if not (hasattr(test_buffer, 'closed') and test_buffer.closed):
                sys.stdout = io.TextIOWrapper(test_buffer, encoding='utf-8', errors='replace', line_buffering=True)
        except Exception:
            # Any error means we can't modify stdout, keep original
            pass
    
    if sys.stderr is not None and sys.stderr is not sys.__stderr__:
        # Already modified, skip
        pass
    elif hasattr(sys.stderr, 'buffer'):
        try:
            # Test if we can actually access and use the buffer
            test_buffer = sys.stderr.buffer
            if not (hasattr(test_buffer, 'closed') and test_buffer.closed):
                sys.stderr = io.TextIOWrapper(test_buffer, encoding='utf-8', errors='replace', line_buffering=True)
        except Exception:
            # Any error means we can't modify stderr, keep original
            pass
